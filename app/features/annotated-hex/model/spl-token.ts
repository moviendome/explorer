...
// Max characters of user-supplied on-chain text (TokenMetadata name/symbol/uri,
...
export function sanitizeDisplayString(value: string): string {
    ...
    for (const ch of value) {
        ...
        chars.push(isControl || isBidi ? '�' : ch);
    }
...
export function buildSplMintRegions(raw: Uint8Array, parsed: MintAccountInfo | undefined): Region[] {
...
const STATE_LABELS: Record<number, TokenAccountInfo['state']> = {
    0: 'uninitialized',
    1: 'initialized',
    2: 'frozen',
};
...
export function buildSplTokenAccountRegions(
    raw: Uint8Array,
    parsed: TokenAccountInfo | undefined,
): Region[] {
    if (raw.length < SPL_TOKEN_ACCOUNT_SIZE) {
...
// Numeric discriminants for SPL Token-2022 extensions (on-chain TLV layer).
// Keep in sync with the string enum `ExtensionType` in
// `app/validators/accounts/token-extension.ts` (jsonParsed layer). Source of
// truth for both: `spl-token-2022::extension::ExtensionType` (Rust).
/* eslint-disable sort-keys-fix/sort-keys-fix -- numeric-keyed map; natural-order reads better than lexicographic */
...
export function walkTokenExtensions(raw: Uint8Array, baseSize: number): Region[] {
...
function decodeMintCloseAuthority(raw: Uint8Array, start: number, length: number, extIndex: number): Region[] {
    if (length < 32) return [];
    return [
        {
...
function decodePermanentDelegate(raw: Uint8Array, start: number, length: number, extIndex: number): Region[] {
    if (length < 32) return [];
    return [
        {
            decodedValue: decodeOptionalNonZeroPubkey(raw, start),
...
function decodeMetadataPointer(raw: Uint8Array, start: number, length: number, extIndex: number): Region[] {
    if (length < 64) return [];
    return [
        {
            decodedValue: decodeOptionalNonZeroPubkey(raw, start),
...
function decodeTokenMetadata(raw: Uint8Array, start: number, length: number, extIndex: number): Region[] {
